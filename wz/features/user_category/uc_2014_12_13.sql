-- 对用户-分类的行为聚集函数
drop table if exists user_category_behavior_agg_2014_12_13;
create table user_category_behavior_agg_2014_12_13 as
select
    -- basic info.
    user_id,
    item_category,
    ...
    -- min
    min(case when behavior_type=1 then cast(substr(time, -2) as bigint) else 25 end) min_bws_hr,
    min(case when behavior_type=2 then cast(substr(time, -2) as bigint) else 25 end) min_clt_hr,
    min(case when behavior_type=3 then cast(substr(time, -2) as bigint) else 25 end) min_crt_hr,
    min(case when behavior_type=4 then cast(substr(time, -2) as bigint) else 25 end) min_buy_hr,
